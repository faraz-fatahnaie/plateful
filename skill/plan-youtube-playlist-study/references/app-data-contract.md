...
`topicOrganization` records the preferred method (`publisher`, `ai`, or
`manual`), the most recently generated baseline, whether publisher segments
were detected, and whether the user edited the result. Each video may preserve
`publisherTopic` separately from its editable `topic` and records
`topicSource`.
...
- Default topic extraction to publisher sections/chapters when meaningful.
- Treat publisher and AI classifications as editable baselines. Preserve all
  user topic names, priorities, ordering, and episode reassignments.
