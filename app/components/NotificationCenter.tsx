...
export default function NotificationCenter({
  ...
  const [preferences, setPreferences] = useState(project.notificationPreferences || defaultPreferences);
  ...
  function savePreferences() {
    onUpdate({ ...project, notificationPreferences: preferences, updatedAt: new Date().toISOString() });
  ...
  async function sendTest() {
    ...
    try {
      const response = await fetch("/api/notifications/email", {
        ...
        body: JSON.stringify({ to: preferences.emailAddress, subject: "Your Plateful reminder is ready", message: `Your next ${project.title} study session starts at ${project.policy.startTime}.` }),
      });
    ...
    <div className="notification-layer" role="presentation" onClick={onClose}>
      <aside className="notification-center" role="dialog" aria-modal="true" aria-label="Notifications" onClick={(event) => event.stopPropagation()}>
        ...
        {!settings ? <>
          ...
          <label>Email address<input type="email" value={preferences.emailAddress} onChange={(event) => setPreferences({ ...preferences, emailAddress: event.target.value })} placeholder="you@gmail.com" /></label>
          ...
          <p className="email-privacy">No Gmail password is stored. Your server sends through an authorized mail provider; the Gmail address is only the recipient.</p>
