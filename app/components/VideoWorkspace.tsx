...
import { AI_PROVIDER_CATALOG, getAIProvider, providerAllowsKey, providerDefaults as getProviderDefaults, providerUsesManualHandoff } from "../../lib/ai-providers";
import { buildStudyPrompt, parseStudyArtifacts, STUDY_PROMPT_PRESETS, type StudyPromptPresetId } from "../../lib/ai-study";
import { dateInTimezone, formatDuration } from "../../lib/playlist-study";
...
export default function VideoWorkspace({
  ...
  const today = dateInTimezone(project.policy.timezone);
  const session = project.sessions.find((item) => item.date >= today && item.status === "planned" && item.videoIds.includes(video.id))
    || [...project.sessions].reverse().find((item) => item.videoIds.includes(video.id));
  ...
  async function loadTranscript() {
    ...
    try {
      const response = await fetch("/api/videos/transcript", {
        ...
        body: JSON.stringify({ url: video.url, language: navigator.language }),
      });
      const payload = (await response.json()) as { transcript?: string; language?: string; error?: string };
      ...
      setTranscriptLanguage(payload.language || "YouTube captions");
      ...
      setMessage(`${payload.language || "YouTube"} transcript loaded automatically${persistTranscript ? " and saved" : " for this session only"}. Review it before generating study material.`);
    } catch (error) {
      setMessage(error instanceof Error ? `${error.message}${transcript.trim() ? " Your saved transcript was kept." : ""}` : "Captions unavailable. Paste a transcript instead.");
    } finally {
  ...
  async function copyManualRequest() {
    ...
    try {
      ...
      setMessage(`Prompt copied. Paste it in ${getAIProvider(provider).shortLabel}, then bring the JSON response back here.`);
    } catch {
  ...
  function prepareManualHandoff(event: MouseEvent<HTMLAnchorElement>) {
    if (!transcript.trim()) {
      event.preventDefault();
      setMessage("Load captions or paste a transcript first.");
      return;
    }
    void copyManualRequest();
  }

  function importManualResponse() {
    try {
      const manualProvider = getAIProvider(provider);
      const artifacts: AIStudyArtifacts = { ...parseStudyArtifacts(manualResponse), provider: `${provider}-manual`, model: model || manualProvider.shortLabel, generatedAt: new Date().toISOString(), transcriptSource };
      onSaveVideo({ ...video, transcript: persistTranscript ? transcript : undefined, aiArtifacts: artifacts });
      setMessage(`${manualProvider.shortLabel} study pack imported. Your personal note is still separate and unchanged.`);
    } catch (error) {
      ...
      {tab === "ai" && (
        <div className="ai-studio workspace-panel">
          <article className="ai-control-card">
            ...
            {provider === "arena" && <p className="provider-caveat"><strong>Arena Direct Chat handoff:</strong> Plateful opens arena.ai after copying your edited prompt. Choose Direct Chat there; Agent mode is still available for unusually complex follow-up work.</p>}
            ...
            <div className="transcript-heading"><div><strong>YouTube transcript</strong><small>{transcriptLanguage ? `${transcriptLanguage} · ${transcript.trim().split(/\s+/).filter(Boolean).length} words loaded` : "Fetches this episode’s public captions without leaving Plateful."}</small></div><button type="button" onClick={loadTranscript} disabled={transcriptBusy}>{transcriptBusy ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}{transcriptBusy ? "Loading transcript…" : transcript ? "Reload YouTube transcript" : "Load YouTube transcript"}</button></div>
            ...
            {providerUsesManualHandoff(provider) ? <div className="manual-ai-handoff"><div className="handoff-step"><span>1</span><div><strong>Send the prepared prompt</strong><small>Plateful copies your edited prompt, then continues to {getAIProvider(provider).shortLabel} in this tab. Use Back to return.</small></div></div><a className="primary-button full button-with-icon" href={getAIProvider(provider).defaultBaseUrl} onClick={prepareManualHandoff}><ExternalLink size={15} />Copy prompt & continue to {getAIProvider(provider).shortLabel}</a><button className="text-button copy-only-button" type="button" onClick={copyManualRequest}><Copy size={14} />Copy only</button><div className="handoff-step"><span>2</span><div><strong>Bring the answer back</strong><small>Copy {getAIProvider(provider).shortLabel}&apos;s JSON answer and paste it below. Plateful validates it before saving.</small></div></div><label>{getAIProvider(provider).shortLabel} response<textarea value={manualResponse} onChange={(event) => setManualResponse(event.target.value)} placeholder={'{"summary":"…","keyPoints":[…],"commands":[],"mindMap":[…],"practice":[…],"quiz":[…]}'} dir="auto" /></label><button className="secondary-button full button-with-icon" type="button" onClick={importManualResponse} disabled={!manualResponse.trim()}><Upload size={15} />Import & show result</button></div> : <button className="primary-button full ai-generate-button" type="button" disabled={aiBusy || !promptDraft.trim()} onClick={analyze}>{aiBusy ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}{aiBusy ? "Building your study pack…" : video.aiArtifacts ? "Regenerate with this prompt" : "Send prompt & create study pack"}</button>}
          </article>
